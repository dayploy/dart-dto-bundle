...
class EntityGenerator
{
    ...
    public function writeEntityClass(
        ...
        if ($reflectionClass->isEnum()) {
            $content = $this->enumGenerator->generateEntityClass(
                $reflectionClass,
            );
        } else {
    ...
    private function createDirectories(
        ...
        $parentPath = StringCase::camelToSnake(dirname($targetFileName));
    ...
    private function getTargetedFilename(
        ...
        // remove .php
        $withoutExtension = StringCase::camelToSnake(
            substr($transformedFileName, 0, strlen($transformedFileName) - 4)
        );

        return $withoutExtension.'.dart';
    }
}
